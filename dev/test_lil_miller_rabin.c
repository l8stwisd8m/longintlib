...
Test(test_lil_miller_rabin, empty_value) {
    uint64_t arr_a[LIL_256_BIT] = {0};
    long_int a = {PLUS, arr_a, LIL_256_BIT};
    int flag = lil_miller_rabin(&a, 10);
    cr_expect_eq(flag, LIL_TEST_FAIL);
}

Test(test_lil_miller_rabin, input_is_one) {
...
Test(test_lil_miller_rabin, input_is_two) {
...
Test(test_lil_miller_rabin, empty_rounds_amount) {
    uint64_t arr_a[LIL_256_BIT] = {1234567};
    long_int a = {PLUS, arr_a, LIL_256_BIT};
    int flag = lil_miller_rabin(&a, 0);
    cr_expect_eq(flag, LIL_TEST_FAIL);
}

Test(test_lil_miller_rabin, input_is_small_prime) {
...
Test(test_lil_miller_rabin, input_is_small_even) {
...
Test(test_lil_miller_rabin, input_is_short_composite) {
    uint64_t arr_a[LIL_256_BIT] = {1234567};
    long_int a = {PLUS, arr_a, LIL_256_BIT};
    int flag = lil_miller_rabin(&a, 10);
    cr_expect_eq(flag, LIL_COMPOSITE);
...
Test(test_lil_miller_rabin, input_is_short_prime) {
...
Test(test_lil_miller_rabin, input_is_long_composite) {
...
Test(test_lil_miller_rabin, input_is_long_prime) {
